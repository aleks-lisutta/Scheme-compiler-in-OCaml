...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  |e::s ->
    ...
    | ((true,x),(true,y))-> if (rb || wb || rp || wp) then true else rw_in_tup true x true y s
    | ((true,x),(false,true)) -> if (wp || wb || rb) then true else rw_in_tup true (rp||x) false true s
    | ((true,x), (false,false))-> if (wb || wp) then true else rw_in_tup true (x||rp) false false s
    | ((false,true), (true,y)) -> if (wb || rb ||rp) then true else rw_in_tup false true true (y||wp) s
